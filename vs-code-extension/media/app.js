...
const VSCODE_SECRET_OPENAI_KEY = 'modernEditor.openaiApiKey';
const VSCODE_SECRET_GEMINI_KEY = 'modernEditor.geminiApiKey';
const VSCODE_PERSIST_META_KEY = 'persistApiKeysMeta';
...
function isVsCodeBackendAvailable() {
  return !!(window.__ME_BACKEND__ && typeof window.__ME_BACKEND__.request === 'function');
}

function setVsCodePersistMeta(meta) {
  if (!meta || typeof meta !== 'object') return;
  const payload = {
    openai: !!meta.openai,
    gemini: !!meta.gemini
  };
  payload.hasAny = payload.openai || payload.gemini;
  safeSetStorage(VSCODE_PERSIST_META_KEY, JSON.stringify(payload));
}

function getVsCodePersistMeta() {
  const raw = safeGetStorage(VSCODE_PERSIST_META_KEY);
...
function getPersistedKeysMeta() {
  if (isVsCodeBackendAvailable()) {
    return getVsCodePersistMeta();
  }
...
async function loadPersistedKeys() {
  if (isVsCodeBackendAvailable()) {
    ...
    let meta = getVsCodePersistMeta();
    let openai = '';
    let gemini = '';
    if (backend) {
      try {
        const [openaiValue, geminiValue] = await Promise.all([
          backend.request('secrets.get', { key: VSCODE_SECRET_OPENAI_KEY }),
          backend.request('secrets.get', { key: VSCODE_SECRET_GEMINI_KEY })
        ]);
        openai = typeof openaiValue === 'string' ? openaiValue : '';
        gemini = typeof geminiValue === 'string' ? geminiValue : '';
        meta = { openai: !!openai, gemini: !!gemini, hasAny: !!(openai || gemini) };
        setVsCodePersistMeta(meta);
      } catch (err) {
        console.warn('Failed to load keys from VS Code Secret Storage.', err);
      }
    }
    if (!isPersistKeysEnabled()) {
      return { openai: '', gemini: '', hasAny: meta.hasAny };
    }
    return { openai, gemini, hasAny: meta.hasAny };
  }
...
function savePersistedKeys(openaiKey, geminiKey) {
  if (isVsCodeBackendAvailable()) {
    const backend = window.__ME_BACKEND__;
    const meta = { openai: !!openaiKey, gemini: !!geminiKey };
    setVsCodePersistMeta(meta);
    if (backend) {
      const tasks = [];
      if (openaiKey) {
        tasks.push(backend.request('secrets.set', { key: VSCODE_SECRET_OPENAI_KEY, value: openaiKey }));
      } else {
        tasks.push(backend.request('secrets.delete', { key: VSCODE_SECRET_OPENAI_KEY }));
      }
      if (geminiKey) {
        tasks.push(backend.request('secrets.set', { key: VSCODE_SECRET_GEMINI_KEY, value: geminiKey }));
      } else {
        tasks.push(backend.request('secrets.delete', { key: VSCODE_SECRET_GEMINI_KEY }));
      }
      Promise.all(tasks).catch((err) => {
        console.warn('Failed to persist keys in VS Code Secret Storage.', err);
      });
    }
    return;
  }
...
function clearPersistedKeys() {
  if (isVsCodeBackendAvailable()) {
    setVsCodePersistMeta({ openai: false, gemini: false });
    const backend = window.__ME_BACKEND__;
    if (backend) {
      Promise.all([
        backend.request('secrets.delete', { key: VSCODE_SECRET_OPENAI_KEY }),
        backend.request('secrets.delete', { key: VSCODE_SECRET_GEMINI_KEY })
      ]).catch((err) => {
        console.warn('Failed to clear keys in VS Code Secret Storage.', err);
      });
    }
    return;
  }
...
const vsCodeHelpBtn = document.getElementById('vsCodeHelpBtn');
const vsCodeHelpOverlay = document.getElementById('vsCodeHelpOverlay');
const vsCodeHelpModal = document.getElementById('vsCodeHelpModal');
const vsCodeHelpClose = document.getElementById('vsCodeHelpClose');
...
if (vsCodeHelpBtn) {
  vsCodeHelpBtn.addEventListener('click', () => {
    closeHamburgerMenu();
    showVsCodeHelpModal();
  });
}
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  if (!restored) {
    if (hasVsCodeBackend) {
      // Leave blank and let the VS Code adapter auto-load the active document.
      documentInput.value = '';
      originalDocumentText = '';
      updateHighlightOverlay();
    } else {
...
if (vsCodeHelpClose) vsCodeHelpClose.addEventListener('click', closeVsCodeHelpModal);
if (vsCodeHelpOverlay) vsCodeHelpOverlay.addEventListener('click', closeVsCodeHelpModal);
...
async function checkApiKey() {
  ...
  const persisted = await loadPersistedKeys();
  const persistedSource = isVsCodeBackendAvailable() ? 'secret-storage' : 'local-storage';
  if (!window.OPENAI_API_KEY && persisted.openai) {
    window.OPENAI_API_KEY = persisted.openai;
    keyState.openai.source = persistedSource;
  }
  if (!window.GEMINI_API_KEY && persisted.gemini) {
    window.GEMINI_API_KEY = persisted.gemini;
    keyState.gemini.source = persistedSource;
  }
...
function describeKey(source, hasKey) {
  ...
  if (source === 'secret-storage') return 'Key present (VS Code Secret Storage)';
...
function showVsCodeHelpModal() {
  if (!vsCodeHelpModal || !vsCodeHelpOverlay) return;
  vsCodeHelpModal.classList.add('visible');
  vsCodeHelpOverlay.classList.add('visible');
}

function closeVsCodeHelpModal() {
  if (!vsCodeHelpModal || !vsCodeHelpOverlay) return;
  vsCodeHelpModal.classList.remove('visible');
  vsCodeHelpOverlay.classList.remove('visible');
}
...
// VS Code helper: allow the webview adapter to replace the document and baseline.
window.__ME_VSCODE_IMPORT_TEXT__ = function(text, options = {}) {
  const incoming = typeof text === 'string' ? text : '';
  const format = options && options.format ? options.format : '';
  resetState({ skipSave: true });
  documentInput.value = incoming;
  originalDocumentText = documentInput.value;
  clearUserSelection();
  updateHighlightOverlay();
  if (formatSelect && format) {
    formatSelect.value = format;
    safeSetStorage('formatPref', format);
  }
  sessionDirty = true;
  flushSessionNow();
};
