...
Identify issues and opportunities for improvement in the document and return them in a JSON structure with this schema:
...
* Missing conditions essential for a stated claim to hold.
...
You may also suggest high-value reorganizations or clarifications when they improve logical structure or readability, but avoid rewriting large parts of the document purely for stylistic preference.
...
* If a reference document clearly plays the role of ground truth (e.g., the original paper), use it to check factual, logical, and mathematical correctness.
...
1. The "original" string MUST be copied EXACTLY as it appears in the TARGET DOCUMENT:
   ...
   * Same characters, punctuation, capitalization, whitespace.
...
3. Whenever feasible, choose "original" so that it appears only once in the TARGET DOCUMENT:
   ...
   * If a short phrase is ambiguous (occurs multiple times), include a bit more surrounding context until it is unique.
...
You SHOULD correct and improve:
...
* Mathematical expressions: algebraic mistakes, sign errors, reversed inequalities, missing absolute values, missing conditions.
...
* Rewrite large sections purely to impose stylistic preferences.
* Introduce new concepts, notation, or assumptions unless clearly necessary to make an existing argument correct.
...
* If a formula or derivation is fundamentally flawed and requires a substantial rewrite, use a comment entry instead of attempting to reconstruct it in detail.
...
* "corrected":
  ...
  * Keep LaTeX changes minimal; only change when a symbol or expression is genuinely wrong or confusing.
...
* "explanation":
  ...
  * If you are relying on a reference document, mention this explicitly (e.g., "Fact: According to the reference, Proposition 1 assumes risk neutrality.").
...
* Avoid rewriting entire sections inside "corrected". For large structural problems, use comment entries as described below.
...
* For changes that are interpretive or stylistic (e.g., improving flow, adding clarifying clauses, tightening wording), mark them as optional:
...
* The organization of ideas makes the logic very hard to follow.
...
* "explanation" clearly describes what the author should do manually. Examples:
  ...
  * "This paragraph misstates the main result; rewrite it to match the formulation in Theorem 2."
  * "The proof omits the case (k=0); add a separate argument for this case to make the proof complete."
  * "The derivation of (3.4) is unclear and appears incorrect; rewrite the derivation starting from (3.2)."
* "type" = "comment".
...
* Do NOT normalize whitespace or LaTeX formatting unless it is necessary to correct a genuine error or clearly improve readability.
* Prioritize:
